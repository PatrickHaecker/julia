...
function expand_abstract_or_primitive_type(ctx, ex)
    ...
    layer = new_scope_layer(ctx, name)
    global_name = adopt_scope(name, layer)
    @ast ctx ex [K"block"
        [K"assert" "toplevel_only"::K"Symbol" [K"inert_syntaxtree" ex] ]
        [K"scope_block"(scope_type=:hard)
            [K"global" global_name]
            [K"block"
                [K"local" name]
                ...
                [K"=" name newtype_var]
                # A not-yet-defined supertype (`abstract type A <: B end`
                # with `B` unbound) raises a recoverable `UndefVarError`
                # inside `_setsuper!`. Publish `newtype_var` as an
                # incomplete-type placeholder before letting the C
                # toplevel evaluator defer the surface AST; on retry,
                # `_typebody!`'s 2-arg form finalises that placeholder
                # in place, preserving any reference that captured it.
                [K"trycatchelse"
                        ...
                        [K"block"
                            ...
                            [K"=" hasprev
                                  [K"&&" [K"call" "isdefinedglobal"::K"core"
                                          ...
                                          name=>K"Symbol"
                                          false::K"Bool"]
                                         [K"call" "_equiv_typedef"::K"core" global_name newtype_var]
                                   ]]
                            [K"=" prev [K"if" hasprev global_name false::K"Bool"]]
                            [K"=" newdef [K"call" "_typebody!"::K"core" prev newtype_var]]
                            [K"constdecl" global_name newdef]
                            (::K"latestworld")
                        ]
                    ]
                    [K"scope_block"(scope_type=:neutral)
                        [K"block"
                            [K"if"
                                [K"call" "incomplete_can_defer"::K"top"
                                    [K"call" current_exception::K"Value"]
                                    ctx.mod::K"Value"
                                ]
                                [K"if" [K"call" "isdefinedglobal"::K"core"
                                        ctx.mod::K"Value"
                                        name=>K"Symbol"
                                        false::K"Bool"]
                                    nothing_(ctx, ex)
                                    [K"block"
                                        [K"constdecl" global_name newtype_var]
                                        (::K"latestworld")
                                    ]
                                ]
                            ]
                            [K"call" "rethrow"::K"top"]
                        ]
                    ]
                ]
            ]
    ...
    @ast ctx ex [K"block"
        ...
        [K"scope_block"(scope_type=:hard)
            ...
            [K"block"
                ...
                # Evaluate the field types and finalise the type body. On
                # success the new `DataType` is published. If a field-type
                # expression below raises a recoverable `UndefVarError` (the
                # C toplevel evaluator will defer the surface AST and re-try
                # it once the missing dependency is bound), publish the
                # incomplete `DataType` placeholder beforehand so that
                # mutually-recursive struct definitions can resolve references
                # to this name on the sibling's first try; the eventual retry
                # takes the `hasprev` path above and `_typebody!` finalises
                # this placeholder in place rather than allocating a new
                # `DataType`. Non-recoverable errors propagate without
                # publishing, leaving the binding unset.
                [K"trycatchelse"
                    [K"scope_block"(scope_type=:neutral)
                        [K"block"
                            [K"=" newdef
                            ...
                            [K"constdecl" global_struct_name newdef]
                            (::K"latestworld")
                        ]
                    ]
                    [K"scope_block"(scope_type=:neutral)
                        [K"block"
                            [K"if"
                                ...
                                [K"if" hasprev
                                    nothing_(ctx, ex)
                                    [K"block"
                                        [K"constdecl" global_struct_name newtype_var]
                                        (::K"latestworld")
                                    ]
                                ]
                            ]
