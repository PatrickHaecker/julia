...
# Incomplete-type hooks. Bootstrap-time fallbacks: incomplete-type deferral
# is unsupported (recover returns false; defer rethrows). The full
# implementations are installed in `Base.incompletetypes`.
incomplete_can_defer(@nospecialize(e), mod::Module, @nospecialize(ast)) = false
incomplete_defer(mod::Module, @nospecialize(e), @nospecialize(ast)) = throw(e)
